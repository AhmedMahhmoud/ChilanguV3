...
class _NormalUserReportState extends State<NormalUserReport> {
  ...
  String newString;
  onDateSelectionChange(DateRangePickerSelectionChangedArgs selection) {
    final PickerDateRange val = selection.value;
    setState(() {
      final DateTime now = DateTime.now();

      if (DateTime(now.year, now.month, now.day)
              .difference(DateTime(val.startDate.year, val.startDate.month,
                  val.startDate.day + 31))
              .inDays <=
          0) {
        yesterday = DateTime(now.year, now.month, now.day - 1);
      } else {
        yesterday = DateTime(
            val.startDate.year, val.startDate.month, val.startDate.day + 31);
      }
    });
  }

  void _dateFun(PickerDateRange picked) async {
    setState(() {
    ...
    if (_dateController.text != newString) {
      ...
      await Provider.of<ReportsData>(context, listen: false).getUserReportUnits(
          Provider.of<UserData>(context, listen: false).user.userToken,
          selectedId,
          dateFromString,
          dateToString,
                                                ...
                                                child: Builder(
                                                  builder: (context) {
                                                    return InkWell(
                                                      ...
                                                      onTap: yesterday.isBefore(
                                                                  getMinDateTime()) ||
                                                              DateTime.now()
                                                                      .difference(Provider.of<CompanyData>(
                                                                              context,
                                                                              listen: false)
                                                                          .com
                                                                          .createdOn)
                                                                  ...
                                                                  return StatefulBuilder(
                                                                    ...
                                                                    return Dialog(
                                                                      ...
                                                                      child:
                                                                        ...
                                                                        minDate:
                                                                            getMinDateTime(),
                                                                        onSelectionChangeFun:
                                                                            (s) {
                                                                          setstatee(
                                                                              () {
                                                                            onDateSelectionChange(s);
                                                                          });
                                                                        },
                                                                      ),
                                                                    );
