...
class _AddUserScreenState extends State<AddUserScreen> {
  ...
  void initState() {
    ...
    if (widget.comingFromShifts) {
      ...
      Provider.of<SiteData>(context, listen: false)
          .setDropDownShift(shiftIndex, false);
    } else {
      shiftIndex =
  ...
  fillTextField() async {
    if (widget.isEdit) {
      ...
      Provider.of<SiteData>(context, listen: false).setDropDownShift(
          shiftIndex, false); //to match the -1 bec of all shifts
                                        ...
                                        child: ShiftsDropDown(
                                          ...
                                          selectedvalue: siteShiftData
                                              .dropDownShifts[siteData
                                                  ...
                                                  : 0] //-1 because كل المناوبات is counted in shift index
                                              .shiftName,
                                          textColor: Colors.orange,
                                        ),
                                      ),
                                  ...
                                  if (!widget.isEdit) {
                                    if (_formKey.currentState.validate()) {
                                      if (Provider.of<SiteShiftsData>(context,
                                                  listen: false)
                                              .shifts[Provider.of<SiteShiftsData>(
                                                              context,
                                                              listen: false)
                                                          .shifts
                                                          .length ==
  ...
  Future<bool> onWillPop() async {
    ...
    Provider.of<SiteData>(context, listen: false).setDropDownIndex(0, true);
    Navigator.of(context).pushAndRemoveUntil(
