...
const localURL = "http://192.168.2.50:8010";
// "http://10.0.0.62:8010"
const appLogo = "resources/roundBlack.png";
...
String getVacationType(int holidayType) {
  switch (holidayType) {
    case 1:
      return getTranslated(null, "عارضة");
      break;
    case 2:
      return getTranslated(null, "مرضى");
      break;
    case 3:
      return getTranslated(null, "رصيد اجازات");
      break;
    default:
      return "";
  }
...
DateTime getMinDateTime() {
  final DateTime now = DateTime.now();
  final comProv = locator.locator<CompanyData>();

  return now.year - comProv.com.createdOn.year >= 1
      ? DateTime(now.year - 1, now.month, now.day)
      : DateTime(
          now.year, comProv.com.createdOn.month, comProv.com.createdOn.day - 1);
}
...
class FailiureCenteredMessage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return CenterMessageText(message: getTranslated(context, "حدث خطأ ما"));
  }
}
...
// }
//Navigation
goPage(BuildContext context, page) {
  Navigator.of(context).push(_createRoute(page));
}

goPageReplacment(BuildContext context, page) {
  Navigator.of(context).pushReplacement(_createRoute(page));
}

goAndRemoveUntill(BuildContext context, page) {
