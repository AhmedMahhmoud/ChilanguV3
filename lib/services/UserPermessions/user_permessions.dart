...
class UserPermessions {
  ...
  factory UserPermessions.pending(dynamic json) {
    return UserPermessions(
        date: DateTime.tryParse(json["date"]),
        duration: json["time"],
        permessionType: json["type"],
...
class UserPermessionsData with ChangeNotifier {
  ...
  Future<String> acceptOrRefusePendingPermession(
    ...
    if (response.statusCode == 200) {
      if (decodedResp["message"] == "Success : User Updated!" ||
          decodedResp["message"] == "Fail: user already attend!") {
  ...
  Future<List<UserPermessions>> getFutureSinglePermession(
    ...
    singleUserPermessions =
        await UserPermessionRepoImp().getFutureSinglePermession(userToken);
    permessionDetailLoading = false;
    ...
    if (response is Faliure) {
      ...
      if (decodedResponse["message"] == "Success") {
        ...
        singleUserPermessions = permessionsObj
            .map((json) => UserPermessions.pending(json))
            .toList();
...
//اعطاء اذن
  Future<String> addUserPermession(
    ...
    try {
      isLoading = true;
      notifyListeners();
      final response = await UserPermessionRepoImp().addPermession(
          userPermessions,
          userToken,
          userId,
          userPermessions.date.toString().substring(0, 11));
      isLoading = false;
      notifyListeners();
      ...
      } else if (decodedMsg == "Failed : User already Attend!") {
        return "Failed : User already Attend!";
      } else if (decodedMsg == "Failed : User already Leave!") {
        return "User already Leave";
      } else if (decodedMsg == "Failed : Pending limit reached") {
        return "pending limit exceed";
      } else if (decodedMsg == "Failed: Daily limit reached") {
        return "daily limit exceed";
      } else if (decodedMsg == "Out of shift time!") {
        return "shiftTimeOut";
      } else if (decodedMsg == "Failed : Another permission at this time!") {
        return "intersection permissions";
      } else if (decodedMsg ==
          "Late Attend Permission Must be before leave by One Hour") {
        return "incorrect attend permision time";
      } else if (decodedMsg ==
          "Early Departure Permission Must be after shift start by One Hour") {
        return "incorrect leave permision time";
      } else if (decodedMsg ==
          "Permission Leave Cannot Set Because Another Late Permission Exist On This Day") {
        return "latePermExist";
      }
      notifyListeners();
      return "failed";
    } catch (e) {
