...
import 'package:qr_users/Features(Underconstructing)/User/Data/Model/user_model.dart';
...
class UserData with ChangeNotifier {
  ...
  Future<int> getDeviceOS() async {
    if (Platform.isIOS) {
      return 1;
    } else if (Platform.isAndroid) {
      bool canObtainFirebaseToken = true;
      fcmToken = await firebaseMessaging.getToken().catchError((e) {
        canObtainFirebaseToken = false;
      });
      if (canObtainFirebaseToken) {
        return 2;
      } else {
        final DeviceInfoPlugin deviceInfo = DeviceInfoPlugin();
        final AndroidDeviceInfo androidInfo = await deviceInfo.androidInfo;
        final String deviceType = androidInfo.manufacturer;
        if (deviceType.toLowerCase() == "huawei") {
          return 3;
        }
        return 4;
      }
  ...
  Future<int> loginPost(String username, String password, BuildContext context,
    ...
    //   }
    // }
    osType = await getDeviceOS();
    log("Device OS: " + osType.toString());
      ...
      showDialog(
          ...
          builder: (BuildContext context) {
            return RoundedAlertOkOnly(
                ...
                title: "هاتف غير مدعوم",
                content: "برجاء تحميل التطبيق من المتجر ");
          });
    }

    final response =
        await MemberRepo().getMemberData(username, password, fcmToken, osType);
    if (response is Faliure) {
      ...
      isTdsAdmin = decodedToken[rolesConstUrl].toString().contains("TDS_Admin");
      isTechnicalSupport =
      ...
      if (decodedRes["message"] == "Success : ") {
        ...
        if (isSuperAdmin) {
          ...
          final List<SuperCompaniesModel> tempComp =
        ...
        log("company image : ${comImageFilePath.toString()}");
  ...
  Future<String> attendByCard({File image, String cardCode}) async {
...
//if (!mounted) return;
  ...
  logout() async {
    ...
    await CachedNetworkImage.evictFromCache(
        locator.locator<UserData>().user.userImage);
    await CachedNetworkImage.evictFromCache(
        locator.locator<CompanyData>().com.logo);
