...
class ShiftsData with ChangeNotifier {
  ...
  Future<bool> getFirstAvailableSchedule(String userId) async {
    isLoading = true;
    notifyListeners();
    final response = await ShiftsRepoImp().getFirstAvailableSchedule();
    isLoading = false;
  ...
  Future<String> addShiftSchedule(
    ...
    final decodedResponse = json.decode(response.body);
    if (decodedResponse["statusCode"] == 200 ||
        decodedResponse["statusCode"] == 400) {
      log(decodedResponse["message"].toString());
      switch (decodedResponse["message"]) {
        case "Success":
          return "Success";
          break;
        case "Fail: Schedle Shift exist for this user!":
          return "exists";
          break;
        case "Fail: User has a mission in this period!":
          return "missionExists";
          break;
        case "First day of schedule Start Before original shift End":
          return "خطأ : يبدأ أول يوم فى الجدولة قبل انتهاء المناوبة الأساسية";
          break;
        case "Last day of schedule Ends After original shift start":
          return "خطأ : ينتهى اخر يوم فى الجدولة بعد بداية المناوبة الأساسية";
          break;
        case "Sunday Start Before Saturday End":
          return "خطأ : يبدأ يوم الأحد قبل إنتهاء يوم السبت";
          break;
        case "Monday Start Before Sunday End":
          return "خطأ : يبدأ يوم الأثنين قبل إنتهاء يوم الأحد";
          break;
        case "Tuesday Start Before Monday End":
          return "خطأ : يبدأ يوم الثلاثاء قبل إنتهاء يوم الأثنين";
          break;
        case "Wednesday Start Before Monday End":
          return "خطأ : يبدأ يوم الأربعاء قبل إنتهاء يوم الثلاثاء";
          break;
        case "Thursday Start Before Wednesday End":
          return "خطأ : يبدأ يوم الخميس قبل إنتهاء يوم الأربعاء";
          break;
        case "Friday Start Before Thursday End":
          return "خطأ : يبدأ يوم الجمعة قبل إنتهاء يوم الخميس";
          break;
        case "Saturday Start Before Friday End":
          return "خطأ : يبدأ يوم السبت قبل إنتهاء يوم الجمعة";
          break;
        default:
          "error";
      }
    ...
    try {
      ...
      if (decodedRes["message"] == "Success") {
        ...
        Provider.of<SiteShiftsData>(context, listen: false).dropDownShifts =
            Provider.of<SiteShiftsData>(context, listen: false)
                .siteShiftList
                .firstWhere((element) => element.siteName == siteName)
                .shifts;
        notifyListeners();
        return "Success";
      } else if (decodedRes["message"] ==
        ...
        return "exists";
      } else if (decodedRes["messa[ge"] == "Fail : Time constants error") {
        debugPrint("s");
  ...
  editShift(Shift shift, int id, String usertoken, BuildContext context,
    ...
    if (response.statusCode == 401) {
      ...
      final decodedRes = json.decode(response.body);
      log(decodedRes["message"].toString());
      if (decodedRes["message"] == "Success") {
        final Shift newShift = Shift(
            fridayShiftenTime:
                int.parse(decodedRes['data']["fridayShiftEntime"]),
