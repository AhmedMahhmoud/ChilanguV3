...
class UserHolidays {
  ...
  factory UserHolidays.fromJsonPending(dynamic json) {
    return UserHolidays(
        fromDate: DateTime.tryParse(json["fromdate"]),
        toDate: DateTime.tryParse(json["toDate"]),
        holidayType: json["typeId"],
...
class UserHolidaysData with ChangeNotifier {
  ...
  getPendingCompanyHolidays(int companyId, String userToken) async {
    ...
    pageIndex++;
    await pendingCompanyHolidays.addAll(await UserHolidaysRepoImplementer()
  ...
  Future<List<UserHolidays>> getFutureSingleUserHoliday(
    ...
    singleUserHoliday = await UserHolidaysRepoImplementer()
        .getFutureSingleUserHoliday(userToken);
    loadingHolidaysDetails = false;
    notifyListeners();
  ...
  Future<Holiday> addHoliday(
    ...
    log(holiday.fromDate.toString().substring(0, 11));
    log(holiday.toDate.toString().substring(0, 11));
    final response = await http.post(
        ...
        body: json.encode({
          ...
          "fromdate": holiday.fromDate.toString().substring(0, 11).trim(),
          "toDate": holiday.toDate.toString().substring(0, 11).trim(),
          "userId": userId,
    ...
    switch (decodedMessage) {
      case "Success : Holiday Created!":
        ...
        break;
      case "Failed : user has an external mission":
      ...
      case "Failed : Another Holiday not approved for this user!":
        ...
        break;
      case "Failed : You have exceeded no of allowed holidays!":
        return Holiday.Holiday_Limit_Exceed;
        break;
      ...
      case "Failed : There are an internal Mission in this period!":
        ...
        break;
      case "Failed: there is a Permisson in this period!":
        return Holiday.Permession_InThis_Period;
        break;
      case "Failed : User Already attended!":
        return Holiday.USER_ALREADY_ATTENDED;
        break;
      case "Failed: Daily limit reached":
        return Holiday.Daily_limit_Reached;
      case "Failed : Pending limit reached":
        return Holiday.Pending_limit_reached;
      default:
  ...
  addExternalMission(DateTime fromDate, DateTime toDate, BuildContext context,
    ...
    if (fromDate != null && toDate != null) {
      ...
      } else if (msg == "Failed : user has a holiday!") {
        displayErrorToast(
            context, "يوجد اجازة تم الموافقة عليها فى هذه الفترة");
      } else {
        displayErrorToast(context, "خطأ في اضافة المأمورية");
      }
    } else {
      Fluttertoast.showToast(
          msg: getTranslated(
