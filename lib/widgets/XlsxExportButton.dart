...
class _XlsxExportButtonState extends State<XlsxExportButton> {
  ...
  Future<void> generateDailyReportExcel(String freePath) async {
    ...
    for (int i = lastIndex; i < (lastIndex + length); i++) {
      ...
      sheet.getRangeByName('B$i').setText(reportProvider
                      .attendListUnits[i - lastIndex].timeOut ==
                  "-" &&
              (reportProvider.attendListUnits[i - lastIndex].timeIn != "-" &&
                  reportProvider.attendListUnits[i - lastIndex].timeIn !=
                  ...
                  reportProvider.attendListUnits[i - lastIndex].timeIn !=
                      getTranslated(context, "غير مقيد بعد")) &&
              reportProvider.attendListUnits[i - lastIndex].timeIn != "-"
          ? DateTime.now().difference(DateTime.parse(widget.day)).inDays == 0
              ? "-"
              : "غياب"
    ...
    if (DateTime.now().difference(DateTime.parse(widget.day)).inDays != 0) {
  ...
  Future<void> generateAbsenceReportExcel(String freePath) async {
    try {
      ...
      sheet.getRangeByName('E1:G1').columnWidth = 7.50;
      sheet.getRangeByName('H1').columnWidth = 7.50;
      sheet.getRangeByName('I1').columnWidth = 4.46;
      sheet.getRangeByName('A1:I1').cellStyle.backColor = '#000000';
      sheet.getRangeByName('A2:I2').cellStyle.backColor = '#000000';
      sheet.getRangeByName('A3:I3').cellStyle.backColor = '#000000';

      sheet.getRangeByName('B1:I3').merge();

      sheet.getRangeByName('B1').setText('Chilangu');
      sheet.getRangeByName('B1').cellStyle.fontSize = 24;
      sheet.getRangeByName('B1').cellStyle.fontColor = '#FF9800';
      sheet.getRangeByName('B1').cellStyle.hAlign = HAlignType.center;
      final Range range0 = sheet.getRangeByName('E4:H6');
      range0.merge();
      range0.cellStyle.hAlign = HAlignType.right;

      sheet.getRangeByName('E4').setText(widget.title);
      sheet.getRangeByName('E4').cellStyle.fontSize = 20;

      final Range range1 = sheet.getRangeByName('F8:H8');
      final Range range2 = sheet.getRangeByName('F9:H9');

      final Range range3 = sheet.getRangeByName('E10:H10');
      final Range range4 = sheet.getRangeByName('E11:H11');
      ...
      sheet.getRangeByName('F8').setText('عن موقع');
      sheet.getRangeByName('F8').cellStyle.fontSize = 12;
      sheet.getRangeByName('F8').cellStyle.bold = true;

      sheet.getRangeByName('F9').setText('${widget.site}');
      sheet.getRangeByName('F9').cellStyle.fontSize = 9;

      sheet.getRangeByName('E10').setText('عن فترة');
      range3.cellStyle.fontSize = 12;
      sheet.getRangeByName('E10').cellStyle.bold = true;

      sheet.getRangeByName('E11').setText(widget.day);

      range4.cellStyle.fontSize = 9;

      sheet.getRangeByIndex(13, 6, 13, 8).merge();

      sheet.getRangeByIndex(13, 6).setText('الاســم');

      sheet.getRangeByName('E13').setText('التأخير');
      sheet.getRangeByName('D13').setText('ايام التأخير');
      sheet.getRangeByName('C13').setText('ايام الغياب');
      sheet.getRangeByName('B13').setText('إجمالى الخصومات');
      sheet.getRangeByName('B13:H13').cellStyle.hAlign = HAlignType.center;
      sheet.getRangeByName('B13:H13').cellStyle.fontSize = 10;
      sheet.getRangeByName('B13:H13').cellStyle.bold = true;
      ...
      for (int i = lastIndex; i < (lastIndex + length); i++) {
        sheet.getRangeByIndex(i, 6, i, 8).merge();

        sheet.getRangeByIndex(i, 6).setText(
        ...
        sheet.getRangeByName('D$i').setText(reportProvider
        ...
        sheet.getRangeByName('B$i').setText(reportProvider
            .lateAbsenceReportUnitList[i - lastIndex].totalDeduction
            .toStringAsFixed(2));

        sheet.getRangeByName('B$i:E$i').cellStyle.hAlign = HAlignType.center;
        sheet.getRangeByName('B$i:H$i').cellStyle.fontSize = 10;
      }
  ...
  Future<void> generateUserReportExcel(String freePath) async {
    String convertStringToVac(String timeIn) {
      switch (timeIn) {
        case "1":
          return "عارضة";
          break;
    ...
    for (int i = lastIndex; i < (lastIndex + length); i++) {
      ...
      sheet.getRangeByName('B$i').setText(reportProvider
                      .userAttendListUnits[i - lastIndex].timeOut ==
                  "-" &&
              (reportProvider.userAttendListUnits[i - lastIndex].timeIn !=
                      "-" &&
                  reportProvider.userAttendListUnits[i - lastIndex].timeIn !=
...
// --------------------------------------------------------------------------
    ...
    sheet.getRangeByName('B${lastIndex + 2}').setText(
          reportProvider.totalLateDuration,
        );
    sheet.getRangeByName('B${lastIndex + 2}').cellStyle.fontSize = 12;
    sheet.getRangeByName('B${lastIndex + 2}').cellStyle.bold = true;
// ------------------------------------------------------------------------
